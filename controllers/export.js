  ...
  if (req.user.sanitary || req.user.admin) {
    models.User.findById(req.user._id)
      ...
      .then(f => {
        ...
        for (var fCounter = 0; fCounter< f.sanitary.floors.length; fCounter++) {
          const titleIndex = 1;
          ...
          for (let rCounter = 0; rCounter < rooms.length; rCounter++) {
              ...
              ws.cell(rCounter+1+titleIndex, titleIndex).string(rooms[rCounter].room);
          ...
          for (let rCounter = 0; rCounter < rooms.length; rCounter++) {
            ...
            for (var i = 0; i < dates.length; i++) {
              if (room.sanitation[dates[i]]) {
                ws.cell(titleIndex+1+rCounter, i+1+titleIndex).number(room.sanitation[dates[i]]);
              } else {
